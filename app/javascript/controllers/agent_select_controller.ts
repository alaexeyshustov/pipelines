...
export default class AgentSelectController extends Controller {
  ...
  async loadVersions(event: Event) {
    ...
    this.snapshotButtonTarget.textContent =
      '+ Create new version from current agent settings';
    ...
    const versions: { id: number; version: number; active: boolean }[] =
      await response.json();
  ...
  async snapshot(event: Event) {
    ...
    if (response.ok) {
      const { id, version } = (await response.json()) as {
        id: number;
        version: number;
      };
      const opt = document.createElement('option');
  ...
  private isPanelOpen(): boolean {
    return (
      this.hasEditPanelTarget &&
      !this.editPanelTarget.classList.contains('hidden')
    );
  }
  ...
  private async loadPromptContent(promptId: string) {
    ...
    this.outputSchemaFieldTarget.value = data.output_schema
