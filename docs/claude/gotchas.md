...
- **Known, unresolved limitation (2026-07-11):** under the per-worker DB isolation above, a small, fully deterministic subset of mutations (~3.2% in a representative scoped run, always mutant's "neutral"-type self-verification mutations) shows `ActiveRecord::StatementInvalid: Could not find table`, traced to the worker's private DB file intermittently reading back as `File.exist? == true` but `File.size == 0` inside a nested "killfork". Root cause not fully found despite substantial investigation (see `spec/mutant_helper.rb`'s "KNOWN LIMITATION" comment for what was ruled out). Do not raise `MUTATION_SCORE_THRESHOLD` until this is resolved or otherwise accounted for — the measured score includes this noise.
