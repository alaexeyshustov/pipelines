...
- **Resolved (2026-07-12):** `.mutant.yml`'s `matcher.ignore:` list was silently non-functional since 2026-05-21. A bare class name (no trailing `*`, e.g. `Emails::Adapters::GmailAdapter`) parses as `Mutant::Expression::Namespace::Exact`, whose `prefix?` requires the *full* expression string to equal the subject's — never true for a method subject like `Emails::Adapters::GmailAdapter#modify_labels`. A trailing `*` (e.g. `Emails::Adapters::GmailAdapter*`) parses as `Mutant::Expression::Namespace::Recursive`, which correctly prefix-matches every subject under that namespace. Every entry in `matcher.ignore:` needs a trailing `*` to actually exclude a class's methods, exactly like every entry in `matcher.subjects:` already has one. Verify this class of bug with a live probe rather than trusting the YAML: `Mutant::Config::DEFAULT.expression_parser.call("SomeClass*").from_right.prefix?(Mutant::Config::DEFAULT.expression_parser.call("SomeClass#method").from_right)` should be `true`; without the `*` it's `false`.
- While fixing the above, 5 of the ignore entries turned out to be mislabeled as "network noise" for classes with zero I/O (pure MIME/header-parsing and pagination-control logic) — an inline "noise" comment had been trusted without checking. Before blanket-excluding a class from mutation scoring, grep the class for actual I/O/network/LLM-client calls (`Google::Apis`, `Net::IMAP`, `TCPServer`, HTTP clients, LLM SDK calls, etc.); if none exist, it belongs in scope, not in `matcher.ignore:`.
