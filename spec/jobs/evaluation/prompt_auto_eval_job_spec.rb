...
RSpec.describe Evaluation::PromptAutoEvalJob do
  ...
  describe "#perform" do
    ...
    it "sets runner_class and evaluator_classes on the new experiment" do
      described_class.perform_now(prompt_id: new_prompt.id)
      new_exp = Evaluation::Experiment.where(prompt: new_prompt).first
      expect(new_exp.runner_class).to eq("Evaluation::Runners::StubbedAgentRun")
    ...
    it "enqueues Evaluation::ExperimentJob for the new experiment" do
    ...
    context "when no completed experiment exists for the prompt name" do
      ...
      it "does not create an experiment" do
        expect { described_class.perform_now(prompt_id: new_prompt.id) }.not_to change(Evaluation::Experiment, :count)
      end

      it "does not enqueue Evaluation::ExperimentJob" do
        allow(Evaluation::ExperimentJob).to receive(:perform_later)
        described_class.perform_now(prompt_id: new_prompt.id)
        expect(Evaluation::ExperimentJob).not_to have_received(:perform_later)
      end
