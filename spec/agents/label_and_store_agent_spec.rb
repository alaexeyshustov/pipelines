...
RSpec.describe LabelAndStoreAgent, type: :model do
  ...
  it 'sends labeling and storage tools as context' do
    expect(
      a_request(:post, mistral_url).with { |req|
        ...
        %w[get_labels create_label add_labels insert_table_rows read_table_schema get_email].all? { |t| tool_names.include?(t) }
      }
    ).to have_been_made
  end
end
