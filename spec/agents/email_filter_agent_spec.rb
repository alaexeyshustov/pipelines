...
RSpec.describe EmailFilterAgent, type: :model do
  ...
  it 'sends classify and temp file tools as context' do
    expect(
      a_request(:post, mistral_url).with { |req|
        tool_names = JSON.parse(req.body)['tools'].map { |t| t.dig('function', 'name') }
        %w[classify_emails manage_temp_file].all? { |t| tool_names.include?(t) }
      }
    ).to have_been_made
  end
end
