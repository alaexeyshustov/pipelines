module Orchestration
  module Executors
    class EmailsFetcher
      include Orchestration::Executable

      DEFAULT_DATE        = Time.zone.today.to_s
      DEFAULT_PROVIDERS   = %w[gmail yahoo].freeze
      DEFAULT_MAX_RESULTS = 10

      input_schema(
        date:        { "type" => "string" },
        providers:   { "type" => "array", "items" => { "type" => "string" } },
        max_results: { "type" => "integer" }
      )

      def self.call(date: nil, providers: DEFAULT_PROVIDERS, max_results: DEFAULT_MAX_RESULTS, **_kwargs)
        new(date: date, providers: providers, max_results: max_results).execute
      end

      def initialize(date: nil, providers: DEFAULT_PROVIDERS, max_results: DEFAULT_MAX_RESULTS, **_kwargs)
        @date        = date
        @providers   = providers
        @max_results = max_results
      end

      def execute
        parsed_date = Date.parse(@date || DEFAULT_DATE)
        providers   = @providers.nil? ? DEFAULT_PROVIDERS : @providers
        max_results = (@max_results || DEFAULT_MAX_RESULTS).to_i

        { "emails" => fetch_emails_async(providers, parsed_date, max_results) }
      end

      def fetch_emails_async(providers, parsed_date, max_results)
        Sync do
          semaphore = Async::Semaphore.new(5)
          providers.map do |provider|
            semaphore.async do
              Emails.list_messages(provider, max_results:, after_date: parsed_date - 1, before_date: parsed_date)
            end
          end.flat_map(&:wait)
        end
      end
    end
  end
end
