  ...
  module Executors
    class Ingestion
      ...
      def self.call(operations: [], **data)
        new(operations: operations, **data).execute
      end

      def initialize(operations: [], **data)
        @operations = operations
        @data       = data.transform_keys(&:to_s) # : Hash[String, json_object_value]
      end
      ...
      def apply_filter_by_ids(output, op)
        ...
        filtered = filter_items(output, source, ids_from)
        build_updated_output(output, dest, filtered)
      end

      def filter_items(output, source, ids_from)
        ...
        source_items.select { |item| (id = item_id(item)) && allowed_ids.include?(id) }
      end

      def build_updated_output(output, dest, value)
        updated = output.dup
        updated[dest] = value
        updated
      end
      ...
      def apply_merge_by_index(output, op)
        ...
        source_arr = fetch_hash_array(output, source)
        ids_arr    = Array(output.fetch(ids_key, []))
        merged     = merge_items(source_arr, ids_arr, inject)
        build_updated_output(output, dest, merged)
      end

      def fetch_hash_array(output, key)
        Array(output.fetch(key, [])).filter_map { |item| item if item.is_a?(Hash) }
      end

      def merge_items(source_arr, ids_arr, inject)
        count = [ source_arr.size, ids_arr.size ].min
        source_arr.first(count).map.with_index do |item, i|
          row = item.dup
